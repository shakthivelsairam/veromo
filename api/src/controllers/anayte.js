import * as analyteDB from "../db/analyte"


export async function add(req, res) {
    try {
        console.log("controller.analyteDB.add req.body = ", req.body)
        const department = await analyteDB.add(req.body)
...
export async function list(req, res) {
    try {
        const samples = await analyteDB.list()
...
export async function get(req, res) {
    try {
        const sample = await analyteDB.get(req.params.id)
...
export async function rangeget(req, res) {
    try {
        const sample = await analyteDB.rangeget(req.params.id)
        res.status(200).json(sample)
    } catch (err) {
        console.error("controller.analyteDB.get - Error: " + JSON.stringify(err))
        res.status(500).json()
