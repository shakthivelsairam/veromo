...
export async function add(anlyt) {
  ...
  try {
    ...
    if (anlyt.rowid > 0) {
      const sqlQuery =
        "UPDATE analytes set code=?,mnemonicCode=?,name=?,report_name=?,depart_id=?,sample_id=?,container_id=?,method_id=?,uom_id=?,loinc_id=?,lonic_code=?,lonic_desc=?,input_pattern=?,result_type=?,decimal_digit=?,active=? WHERE id=?"
      const sqlResult = await dbPool.query(sqlQuery, [
        anlyt.analytecode,
        anlyt.analytemnemonic,
        anlyt.orderName,
        anlyt.reportName,
        anlyt.sltdepartment,
        anlyt.sltsample,
      ...
      const sqlQuery =
        "INSERT INTO analytes (code,mnemonicCode,name,report_name,depart_id,sample_id,container_id,method_id,uom_id,loinc_id,lonic_code,lonic_desc,input_pattern,result_type,decimal_digit,active,created_by,updated_by) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)"
      const sqlResult = await dbPool.query(sqlQuery, [
        anlyt.analytecode,
        anlyt.analytemnemonic,
        anlyt.orderName,
      ...
      anlyt.rangedata.forEach((ele) => {
        ...
        const sqlQueryrange =
          "INSERT INTO analyte_referance_range (analyte_id,referencerange,gender,agetype,agerange,valuetype,valuerange,valuerangevalue,printablerange,deviceid,method,uom,created_by,updated_by) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?)"
        const sqlrangeResult = dbPool.query(sqlQueryrange, [
          lastinsertid,
          ele.sltrefrange,
          ele.sltgender,
          ele.sltagetype,
      ...
      const sqlQueryassay =
        "INSERT INTO analyte_assay_mapping (analyte_id,equipment_id,assay_code,tenant_id,branch_id,method,uom,uploadable,downlodable,active,created_by,updated_by) values (?,?,?,?,?,?,?,?,?,?,?,?)"
      const sqlassaymapResult = await dbPool.query(sqlQueryassay, [
        lastinsertid,
        anlyt.dmdeviceid,
        anlyt.dmassaycode,
        anlyt.dmtenantid,
...
export async function rangeget(analyteid) {
  ...
  try {
    ...
    if (sqlResult && sqlResult.length > 0) {
      console.log(
        "rangeget************************************ details = " + JSON.stringify(sqlResult)
      )
      return sqlResult
    }
  } catch (err) {
