...
export async function add(depts) {
    ...
    try {
      ...
      if (depts.drowid>0)
      {
        const sqlQuery = 'UPDATE departments set name=?,code=?,displayname=?,active=?,short_code=?,mnemonicCode=?,sequence_no=?,tenant_id=?,isprintable=? WHERE id=?'
        // Tenant id needs to be captured from login user and store it here
        const sqlResult = await dbPool.query(sqlQuery, [depts.ddeptname,depts.ddeptcode,depts.ddispname,depts.dactive,depts.dshortcode,depts.dmnemonic,depts.dseqNo,1,depts.dprintSep,depts.drowid])
        console.log("db.departments.add sqlResult = " + JSON.stringify(sqlResult))
        return sqlResult
      }
      else if (depts.drowid===0)
      {
        const sqlQuery = 'INSERT INTO departments (name,code,displayname,active,short_code,mnemonicCode,sequence_no,tenant_id,isprintable,created_by,updated_by) values (?,?,?,?,?,?,?,?,?,?,?)'
        // Tenant id needs to be captured from login user and store it here
        const sqlResult = await dbPool.query(sqlQuery, [depts.ddeptname,depts.ddeptcode,depts.ddispname,depts.dactive,depts.dshortcode,depts.dmnemonic,depts.dseqNo,1,depts.dprintSep, "system", "system"])
        console.log("db.departments.add sqlResult = " + JSON.stringify(sqlResult))
        return sqlResult
      }
    } catch (err) {
