...
export async function add(facilities) {
  ...
  try {
    ...
    if (facilities.frowid > 0) {
      const sqlQuery =
        "UPDATE facility set fcode=?,name=?,displayname=?,address_line1=?,address_line2=?,city=?,state=?,country=?,mobile_number=?,pincode=?,type=?,proccessing_facility_id=?,short_code=?,is_base=?,base_branch_id=?,active=?,tenant_id=?,updated_by=? WHERE id=?"
      // Tenant id needs to be fetch from login and pass it
      const sqlResult = await dbPool.query(sqlQuery, [
        facilities.fcode,
        facilities.fname,
        facilities.fdisplayname,
        facilities.addline1,
        facilities.addline2,
        facilities.fcity,
        facilities.fstate,
        ...
        facilities.fprocessingfacility,
        facilities.fshortcode,
        facilities.fBase,
        facilities.fbasefacilityid,
        facilities.active,
        1,
        "system",
        facilities.frowid,
      ...
      const sqlQuery =
        "INSERT INTO facility (fcode,name,displayname,address_line1,address_line2,city,state,country,mobile_number,pincode,type,proccessing_facility_id,short_code,is_base,base_branch_id,active,tenant_id,created_by,updated_by) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)"
      // Tenant id needs to be fetch from login and pass it
      const sqlResult = await dbPool.query(sqlQuery, [
        facilities.fcode,
        facilities.fname,
...
export async function lookup() {
  const dbPool = await db.getPool()
  try {
    const sqlQuery = "SELECT id as value,name as label FROM facility"
    const sqlResult = await dbPool.query(sqlQuery)
    if (sqlResult && sqlResult.length > 0) {
