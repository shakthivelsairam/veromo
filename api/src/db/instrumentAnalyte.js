...
export async function add(anlyt) {
  ...
  try {
    ...
    if (anlyt.rowid > 0) {
      const sqlQuery =
        "UPDATE analytes set code=?,name=?,report_name=?,depart_id=?,sample_id=?,container_id=?,method_id=?,uom_id=?,loinc_id=?,lonic_code=?,lonic_desc=?,input_pattern=?,result_type=?,decimal_digit=?,active=? WHERE id=?"
      ...
      const sqlQuery =
        "INSERT INTO analytes (code,name,report_name,depart_id,sample_id,container_id,method_id,uom_id,loinc_id,lonic_code,lonic_desc,input_pattern,result_type,decimal_digit,active,created_by,updated_by) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)"
      const sqlResult = await dbPool.query(sqlQuery, [
        anlyt.analytecode,
        anlyt.orderName,
        anlyt.reportName,
