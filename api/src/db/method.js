...
export async function add(method) {
  ...
  try {
    if (method.rowid > 0) {
      const sqlQuery =
        "UPDATE methods set name=?,code=?,displayname=?,mnemonicCode=?,active=?,tenant_id=? WHERE id=?"
      ...
      const sqlQuery =
        "INSERT INTO methods (name,code,active,displayname,mnemonicCode,tenant_id,created_by,updated_by) values (?,?,?,?,?,?,?,?)"
      // Have to capture tenant id from login session
      const sqlResult = await dbPool.query(sqlQuery, [
        method.name,
        method.code,
