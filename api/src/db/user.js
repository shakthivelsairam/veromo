...
export async function add(user) {
  ...
  try {
    ...
    if (user.rowid > 0) {
      const sqlQuery = `UPDATE user set name=?,username=?,email=?,dob=?,gender=?,religion=?,qualification=?,maritalstatus=?,add1=?,add2=?,
        add3=?,city=?,country=?,state=?,zipcode=?,mobile=?,role=?,created=?,updated=?,created_by=?,updated_by=? WHERE id=?`
      ...
      // Have to keep tenant id from login session and store here
      const sqlQuery = `INSERT INTO user (name,username,email,dob,gender,religion,qualification,maritalstatus,add1,add2,add3,city,country,
          state,zipcode,mobile,role=?,created,updated,created_by,updated_by) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`
      // Have to keep tenant id from login session and store here
      const sqlResult = await dbPool.query(sqlQuery, [
        sample.samplename,
        sample.samplecode,
...
export async function lookup() {
  const dbPool = await db.getPool()
  try {
    const sqlQuery = "SELECT id as value,name as label FROM user"
    const sqlResult = await dbPool.query(sqlQuery)
    if (sqlResult && sqlResult.length > 0) {
      console.log("users details = " + JSON.stringify(sqlResult))
      return sqlResult
...
export async function rolelookup() {
  const dbPool = await db.getPool()
  try {
    const sqlQuery = "SELECT id as value,name as label FROM roles"
    const sqlResult = await dbPool.query(sqlQuery)
    if (sqlResult && sqlResult.length > 0) {
      console.log("roles details = " + JSON.stringify(sqlResult))
      return sqlResult
    }
  } catch (err) {
