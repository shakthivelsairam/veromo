...
export default function TenantList(){
  ...
  useEffect(() => {
    (async () => {
       const allrows = await api.getInstrumentData()
      setData(allrows)
    })()
  }, [showFormMetadata])
      ...
      <React.Fragment>
        ...
        <Grid item xs={8}>
            ...
            <Grid item xs={4} style={{textAlign:"right"}}>
              <Button variant="contained"  onClick={()=>pageType(false,0)} style={{marginRight:10}}>Add Instrument Type</Button>
              <Button variant="contained" onClick={()=>metaType(false,0)}>Add</Button>
            </Grid>
            <InstrumentTypes showForm={showForm} editrow={row} togglePage={togglePage}/>
            <InstrumentForm showFormMetadata={showFormMetadata} editmetadata={metaRow} togglePageMeta={togglePageMeta}/>
          ...
          <Table size="small">
            <TableHead>
              <TableRow>
                ...
                <StyledTableCell>Maufacturer</StyledTableCell>
            ...
            {data?data.map((row:any) => (
                <StyledTableRow key={row.id}>
                  ...
                  <StyledTableCell>{row.model_number}</StyledTableCell>
