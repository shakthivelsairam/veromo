...
import { Grid, TextField, FormControlLabel,Checkbox,Button,Typography,MenuItem,Select,InputLabel,FormControl,Autocomplete, Table, TableBody, TableHead, TableRow, Dialog, DialogActions, DialogContent, DialogTitle } from '@mui/material';
...
function PriceMasterForm(props: any){
  ...
  useEffect(() => {
    (async () => {
     ...
     const tariffList = tariffCards.map((tariff: any) => ({"label": tariff.name, "value": tariff.id}))
  ...
  const fetchExistingPriceList = async () => {
    ...
    const existingPriceList = await api.getPrices(tariffCard.value, selectedTest.value)
  ...
  return(
      <React.Fragment>
        <Dialog fullWidth={true} maxWidth={false} open={props.showForm}>
            <DialogTitle className={custstyle.addeditmenu}>{props.editrow ? "Edit" : "Add"} Prices</DialogTitle>
      ...
      <Grid container spacing={3}>
        ...
        <Autocomplete
            ...
            sx={{ width: 300 }}
            renderInput={(params) => <TextField {...params} label="Tariff Card" variant="standard" />}
        ...
        <Grid item xs={3}>
          <Button variant="contained" style={{backgroundColor:"lightgray", color:"black"}} onClick={fetchExistingPriceList}>Search</Button>
        </Grid>
      </Grid>
      <div style={{ height: 400, width: '100%', marginTop: 5 }}>
          <Table size="small">
            ...
            <TableBody>
              {existingPriceList && Array.isArray(existingPriceList) && existingPriceList.map((row:any, index: number) => (
