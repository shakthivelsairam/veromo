...
import { TableRow, Grid, TextField, FormControlLabel,Checkbox,Button,Typography,MenuItem,Select,InputLabel,FormControl,Autocomplete } from '@mui/material';
...
function TestPrice(props: any){
  ...
  const [selectedScheduleDays, setSelectedScheduleDays] = React.useState<any>(new Array(scheduleDays.length).fill(false))
  ...
  const handleAllScheduleDaysChange = () => {
    const updatedCheckedState = new Array(scheduleDays.length).fill(!selectedAllDays)
  ...
  const handleScheduleDaysChange = (day:any, position: number) => {
    const updatedCheckedState = selectedScheduleDays.map((item:boolean, index: number) =>
    ...
    return(
        ...
        <StyledTableRow key={props.rowId}>
          ...
          <FormControlLabel control={<Checkbox
              ...
              onChange={(e) => {handleAllScheduleDaysChange()}}
              inputProps={{ 'aria-label': 'controlled' }}
            />} label={"All"} />
            {scheduleDays.map((day, index) => (
            <FormControlLabel control={<Checkbox
              checked={selectedScheduleDays[index]}
              onChange={(e) => {handleScheduleDaysChange(e.target.value, index)}}
              inputProps={{ 'aria-label': 'controlled' }}
            />} label={day} />
          ))}</StyledTableCell>
        </StyledTableRow>
