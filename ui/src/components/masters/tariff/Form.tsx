...
import { Grid, TextField, FormControlLabel,Checkbox,Button,Typography,MenuItem,Select,InputLabel,FormControl,Autocomplete, OutlinedInput, ListItemText, Box, Chip, Dialog, DialogActions, DialogContent, DialogTitle } from '@mui/material';
    ...
    return(
        <React.Fragment>
          <Dialog fullWidth={true} maxWidth={false} open={props.showForm}>
              <DialogTitle className={custstyle.addeditmenu}>{props.editrow ? "Edit" : "Add"} Tariff</DialogTitle>
        ...
        <Grid item xs={2}>
          ...
          <Grid item xs={2}>
          <FormControlLabel
              control={<Checkbox color="secondary" name="IsDefaultTariff" checked={isDefaultTariff} onChange={(e) => {setIsDefaultTariff(Boolean(e.target.checked))}} />}
              label="Is default tariff"
            />
          </Grid>
          <Grid item xs={2}>
          <FormControlLabel
              control={<Checkbox color="secondary" name="active" checked={status} onChange={(e) => {setStatus(Boolean(e.target.checked))}} />}
              label="Active"
            />
          </Grid>
        </Grid>
