...
export default function TariffMaster(){
  ...
  useEffect(()=>{
    const rows = [
      {id:1, code: "GENERAL", name: "GENERAL", description:"General tariff", status: "Active" },
      {id:2, code: "L2LRATE", name: "L2LRATE", description:"Lab to lab tariff", status: "Inactive" },
    ];
    setData(rows)
  }, [])
      ...
      <div>
        ...
        <Grid container spacing={2}>
            ...
            <Dialog fullWidth={true} maxWidth={false} open={showForm}>
              <DialogTitle className={custstyle.addeditmenu}>{editForm ? "Edit" : "Add"} Tariff</DialogTitle>
              <DialogContent dividers className={custstyle.popupheight}>
                <TariffMasterForm togglePage={togglePage}/>
              </DialogContent>
              <DialogActions>
