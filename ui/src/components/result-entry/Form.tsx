...
import { Grid,MenuItem,Select,InputLabel,FormControl, TextField, FormControlLabel,Checkbox,Button,Typography,Table, TableBody, TableHead, TableRow,TableCell } from '@mui/material';
...
function ResultEntryForm(props: any){
  ...
  useEffect(()=>{
    const rows = [
      {test_name: "Urea - Serum / Plasma", result: '28', uom: 'mg/dL', range: "15 - 40", status: "" },
    ...
    return(
        <React.Fragment>
          ...
          Result Entry <span style={{fontSize:14, color: "#388cd2"}}>Mr.Vijay (Lab Number - 180304804 |Male|32Y - 08-Nov-2021)</span>
        </Typography>
        <Grid container spacing={3}>
          <Grid item xs={false}>
            <span style={{color:"red", fontWeight: "bold"}}><CropSquareIcon fontSize="small"/><span style={{marginTop:5}}>High</span></span>
            <span style={{color:"orange", fontWeight: "bold"}}><CropSquareIcon fontSize="small"/>Low</span>
          </Grid>
        </Grid>
          <Table size="small">
              <TableHead>
              ...
              <TableBody>
                {resultEntryData.map((row:any) => (
                  <TableRow key={row.id}>
                    ...
                    <TableCell align="center">
                      <Button size="small"><CommentIcon fontSize="small"></CommentIcon></Button>
                      <Button size="small"><HistoryIcon fontSize="small"></HistoryIcon></Button>
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
