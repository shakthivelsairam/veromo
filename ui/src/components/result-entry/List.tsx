...
export default function ResultEntryList(){
  ...
  useEffect(()=>{
    const rows = [
      {id: 1, lab_number: "180304804", patient_name: 'Mr.Vijay', patient_details:"M | 32 Y", priority: "STAT", registration_date: "28-Nov-2021 12:17 PM" },
    ...
    return(
      <div>
        {showForm &&
        <ResultEntryForm togglePage={togglePage}/>
        }
        {showList &&
        <React.Fragment>
              ...
              <TableBody>
                {resultEntryData.map((row:any) => (
                  <StyledTableRow key={row.id}>
                    ...
                    <StyledTableCell align="right">{row.patient_name}<br/>{row.patient_details}</StyledTableCell>
                    ...
                    <StyledTableCell align="right"><Button size="small" onClick={togglePage}><ArrowForwardIosIcon fontSize="small"></ArrowForwardIosIcon></Button></StyledTableCell>
                  </StyledTableRow>
                ))}
              </TableBody>
            </Table>
