...
export async function getTariffCardService(tariffCardId) {
  try {
    console.log("getTariffCardService apiBaseURL = ", apiBaseURL)
    const response = await wrappedFetch(`${apiBaseURL}/tariffcard/${tariffCardId}/services`, {
      method: "GET",
    })
    console.log("getTariffCardService response = ", response)
    if (response.status === 401) {
      throw new Error("Unauthorized")
    } else {
      return response.json()
    ...
    console.error("getTariffCardService error: ", error)
    throw new Error("Unauthorized")
  }
}
